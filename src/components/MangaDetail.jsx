...
import { setMangas, setChapters, setPagination } from "../store/actions/manga";
                ...
                <div className='flex flex-col items-center w-[full] sm:w-[60%]'>
                  <p className='m-4 text-[color:var(--black,#222)] text-2xl not-italic leading-[95.187%]'>{manga.title}</p>
                    ...
                    <div className='flex w-full justify-around items-center'>
                        <p className='shadow-[0px_2px_4px_0px_rgba(0,0,0,0.04)] rounded-[50px] flex items-start gap-2.5 p-3 bg-[color:var(--black,#ffe0df)] text-[#EF8481] text-md not-italic font-medium leading-[95.187%]'>{manga.category_id?.name}</p>
                    ...
                    <div className='flex justify-around items-center content-center m-4'>
                        <button className='shrink-0 bg-white rounded-full m-2 shadow-lg w-[25%] sm:w-[30%] h-[60px] hover:ease-in'><p className='text-[rgba(0,0,0,0.80)] text-center text-[35px] flex flex-col justify-center items-center'>👍</p></button>
                        <button className='shrink-0 bg-white rounded-full m-2 shadow-lg w-[25%] sm:w-[30%] h-[60px] hover:ease-in'><p className='text-[rgba(0,0,0,0.80)] text-center text-[35px] flex flex-col justify-center items-center'>👎️</p></button>
                        <button className='shrink-0 bg-white rounded-full m-2 shadow-lg w-[25%] sm:w-[30%] h-[60px] hover:ease-in'><p className='text-[rgba(0,0,0,0.80)] text-center text-[35px] flex flex-col justify-center items-center'>😮</p></button>
                        <button className='shrink-0 bg-white rounded-full m-2 shadow-lg w-[25%] sm:w-[30%] h-[60px] hover:ease-in'><p className='text-[rgba(0,0,0,0.80)] text-center text-[35px] flex flex-col justify-center items-center'>😍</p></button>
                    </div>
                    ...
                    <div className='w-[90%] h-[80px] bg-white rounded-md shadow-lg flex flex-row justify-around items-center'>
                      ...
                      <div className='flex flex-col'>
                          <p className='text-[color:var(--primary-black,#424242)] text-center text-xl not-italic font-normal leading-[normal]'>{totalChapters}</p>
                    ...
                    <div className='w-[90%] h-[40px] shrink-0 flex items-center m-4 shadow-[0px_0px_7px_0px_rgba(0,0,0,0.15)] rounded-[20px]'>
                        <Anchor onClick={switchToMangaTab} className='w-[50%] h-[34.311px] shrink-0 rounded-[20px] bg-orange-500 text-[color:var(--white,#FFF)] text-center text-md not-italic font-normal leading-[normal] flex flex-col justify-center'>Manga</Anchor>
                        <Anchor onClick={switchToChaptersTab} className='flex w-[50%] h-[34px] flex-col justify-center items-center shrink-0 text-[color:var(--secondary-gray,#9D9D9D)] text-center text-md not-italic font-normal leading-[normal]'>Chapters</Anchor>
                    </div>
            ...
            {activeTab === "chapters" && (
                <div className='sm:w-[60%] flex flex-col items-center w-[90%] mb-12'>
                  ...
                  {chapters.map((chapter) => (
                    <div key={chapter._id} className="flex w-[90%] justify-between items-center m-3">
                      ...
                      <div className="flex w-[25%] h-12 flex-col justify-center shrink-0 bg-[color:var(--primary-two-design,#F97316)] rounded-[25px] hover:bg-orange-800 ">
                        <Anchor to={`/chapter/${chapter._id}`} className="text-[#FAFCFC] text-center text-md not-italic font-bold leading-[normal] tracking-[0.7px">Read</Anchor>
                      </div>
                  ...
                  <div className="flex flex-wrap justify-center mt-4">
                    ...
                    <button onClick={handleNextPage} disabled={pagination.currentPage == totalPages} className={`px-4 py-2 mr-2 rounded-md ${pagination.currentPage == totalPages ? 'bg-slate-50' : 'bg-orange-400' }`}> Next </button>
                  </div>
                </div>
              )
            }
